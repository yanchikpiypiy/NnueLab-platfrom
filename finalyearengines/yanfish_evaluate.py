...
class NNUEAccumulator:
    def __init__(self, weights_file="weights/transformer_weights.npy", bias_file="weights/transformer_bias.npy"):
...
class MyNNUE(nn.Module):
    ...
    def forward(self, x):
        ...
        # If your weights are truly int-based, you may want to replicate SHIFT/clamp in each layer,
        # or if they've been dequantized to float, you can do normal float ops.
    ...
    def load_extracted_weights(self):
        w1 = np.load("numpy_weights/hidden_layer_1_weights.npy")   # shape (32, 512)
        b1 = np.load("numpy_weights/hidden_layer_1_bias.npy")      # shape (32,)
        w2 = np.load("numpy_weights/hidden_layer_2_weights.npy")   # shape (32, 32)
        b2 = np.load("numpy_weights/hidden_layer_2_bias.npy")      # shape (32,)
        w_out = np.load("numpy_weights/output_layer_weights.npy")  # shape (1, 32)
        b_out = np.load("numpy_weights/output_layer_bias.npy")     # shape (1,)

        # If these are truly quantized, you might need to shift or scale them. For now, we treat them as float.
