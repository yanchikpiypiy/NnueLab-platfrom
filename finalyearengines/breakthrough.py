...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, kings, accum_root, accum_up, pos_prev, move_prev, root=True):
        ...
        nnue = MyNNUE(compute_piece_count(pos))
        # nnue.eval()
            ...
            if depth == 0:
                if not self.use_classical:
                    with torch.no_grad():
                        ...
                        x = torch.tensor(cur_accum)
                        score_tensor = nnue(accum=x, us=us, psqt_indices=computed_index, layer_stack_indices=computed_index)
                    # Convert the output to a Python float and apply final scaling.
                    raw_score = int(score_tensor.item())
                    # score = ((raw_score // 8) * 100) // 208
        ...
        if(not self.use_classical):      
          ...
          if accum_up:                 
              ...
              cur_accum[0][:512] = np.sum(nnue_accum.weights[ind[0]],axis=0)
              cur_accum[0][512:520] = np.sum(nnue_accum.psqt[ind[0]],axis=0)
              cur_accum[0][520:520+512] = np.sum(nnue_accum.weights[ind[1]],axis=0)
              cur_accum[0][520+512:] =  np.sum(nnue_accum.psqt[ind[1]],axis=0)
              cur_accum[0][:512] += nnue_accum.bias
              cur_accum[0][520:520+512] += nnue_accum.bias
