...
def main():
    ...
    searcher = Searcher()
    start = time.time()
    for depth, move, score, extra in searcher.search(hist[-1], 1.0, history=hist):
        elapsed = time.time() - start
        print(f"Depth: {depth}, Score: {score}, Nodes: {searcher.nodes}, Elapsed: {elapsed:.3f} seconds")
        if elapsed > 1.0:
            break
    ...
    while True:
        ...
        hist.append(hist[-1].move(move))
def play_until_mate(pos, searcher, movetime=1.0, max_depth=100):
    """
    Let the engine self-play from `pos` until it reaches mate (or a terminal position).
    
    :param pos: A yanfish Position (the side to move is derived from pos.board).
    :param searcher: An instance of yanfish.Searcher
    :param movetime: Time limit in seconds to search each move (if you wish).
    :param max_depth: Safety limit on how deep to search in case mate is far away.
    
    :return: (moves, final_pos, is_checkmate) 
       moves: list of moves in UCI form like ["e2e4", "e7e5", ...]
       final_pos: the final position object
       is_checkmate: boolean indicating if we actually checkmated or ended by stalemate/no legal moves
    """
    moves_played = []
    current_pos = pos

    while True:
        # 1) Generate all legal moves from current_pos
        legal_moves = list(current_pos.gen_moves())
        if not legal_moves:
            # No moves => either checkmate (score <= -MATE_LOWER) or stalemate
            is_checkmate = (current_pos.score <= -MATE_LOWER)
            return moves_played, current_pos, is_checkmate

        # 2) Use iterative deepening search to find the best move
        best_move = None
        best_score = None
        start_time = time.time()
        
        for depth, move, score, from_book in searcher.search(current_pos, movetime, history=(), use_classical=False):
            # We'll keep updating best_move, best_score as we go deeper,
            # but break if we run out of time or exceed max_depth
            if depth > max_depth:
                break
            best_move = move
            best_score = score
            if (time.time() - start_time) > movetime:
                break

        if best_move is None:
            # Engine couldn't find a move (unlikely), break out
            return moves_played, current_pos, False
        
        # 3) Apply the move to get the next position
        next_pos = current_pos.move(best_move)

        # Convert the internal (i, j) to a UCI string, e.g. "e2e4"
        move_uci = move_render(current_pos, best_move)
        moves_played.append(move_uci)

        # 4) Advance
        current_pos = next_pos
        # Now the loop continues until mate or no moves


def test_play_until_mate():
    # 1) Create a position (for example, a known mate in 3)
    fen = "r1bqkbnr/pppppppp/n7/1B6/4P3/8/PPPP1PPP/RNBQK1NR b KQkq - 1 2"
    
    # 2) Create the Position object. If you have a `fromFEN` method, use that;
    #    otherwise, write a small helper that sets up the position from FEN.
    pos = Position.fromFEN(fen)  # or however you do it
    
    # 3) Create a Searcher
    searcher = Searcher()
    
    # 4) Call play_until_mate with some time/depth constraints
    moves_line, final_pos, is_checkmate = play_until_mate(
        pos,
        searcher,
        movetime=1.0,   # search up to 1 second each move
        max_depth=50    # just in case
    )
    
    # 5) Print out results
    print("Moves played:", moves_line)
    print("Final position FEN:", renderFEN(final_pos))
    print("Is it checkmate?", is_checkmate)

if __name__ == "__main__":
    test_play_until_mate()
