...
import sys
import chess
import engines.yanfish as yanfish

################################################################################
# This module contains functions used by test.py and xboard.py.
# Nothing from here is imported into yanfish.py which is entirely self-sufficient
################################################################################
...
def can_kill_king(pos):
    ...
    return any(pos.value(m) >= yanfish.MATE_LOWER for m in pos.gen_moves())

def mrender(pos, m):
    # Numbfish always assumes promotion to queen
    p = 'q' if yanfish.A8 <= m[1] <= yanfish.H8 and pos.board[m[0]] == 'P' else ''
    m = m if get_color(pos) == WHITE else (119-m[0], 119-m[1])
    return yanfish.render(m[0]) + yanfish.render(m[1]) + p

def mparse(color, move):
    m = (yanfish.parse(move[0:2]), yanfish.parse(move[2:4]))
    return m if color == WHITE else (119-m[0], 119-m[1])

def renderSAN(pos, move):
    ''' Assumes board is rotated to position of current player '''
    i, j = move
    csrc, cdst = yanfish.render(i), yanfish.render(j)
    # Rotate flor black
    if get_color(pos) == BLACK:
        csrc, cdst = yanfish.render(119-i), yanfish.render(119-j)
    # Check
    pos1 = pos.move(move)
    ...
    # Pawn moves
    if pos.board[i] == 'P':
        pro = '=Q' if yanfish.A8 <= j <= yanfish.H8 else ''
    ...
    srcs_file = [a for a in srcs if (a - yanfish.A1) % 10 == (i - yanfish.A1) % 10]
    srcs_rank = [a for a in srcs if (a - yanfish.A1) // 10 == (i - yanfish.A1) // 10]
...
def parseSAN(pos, msan):
    ...
    for (i, j), _ in gen_legal_moves(pos):
        if get_color(pos) == WHITE:
            csrc, cdst = yanfish.render(i), yanfish.render(j)
        else: csrc, cdst = yanfish.render(119-i), yanfish.render(119-j)
...
def get_color(pos):
    ''' A slightly hacky way to to get the color from a yanfish position '''
...
def parseFEN(fen):
    ...
    ep = yanfish.parse(enpas) if enpas != '-' else 0
    score = sum(yanfish.pst[p][i] for i,p in enumerate(board) if p.isupper())
    score -= sum(yanfish.pst[p.upper()][119-i] for i,p in enumerate(board) if p.islower())
    pos = yanfish.Position(board, score, wc, bc, ep, 0)
...
def renderFEN(pos, half_move_clock=0, full_move_clock=1):
    ...
    ep = yanfish.render(pos.ep) if not pos.board[pos.ep].isspace() else '-'
